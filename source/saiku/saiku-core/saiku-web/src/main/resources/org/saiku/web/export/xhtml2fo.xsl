...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="xhtml:table">
    ...
    <fo:table xsl:use-attribute-sets="tableborder" width="100%"><xsl:call-template name="common-atts"/>
  ...
  <xsl:template match="xhtml:th|th">
    <fo:table-cell xsl:use-attribute-sets="tableborder" font-weight="bold" padding=".1em"><xsl:call-template
            name="common-atts"/>
      <xsl:if test="@colspan">
  ...
  <xsl:template match="xhtml:td|td">
    <fo:table-cell xsl:use-attribute-sets="tableborder" padding=".1em"><xsl:call-template name="common-atts"/>
      <xsl:if test="@colspan">
        <xsl:attribute name="number-columns-spanned"><xsl:value-of select="@colspan"/></xsl:attribute>
      </xsl:if>
      <xsl:if test="@rowspan">
